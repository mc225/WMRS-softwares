classdef Andor < handle
    ...
    %               CCDSettingTemp, by default (-70 Celsius)
    ...
    methods
        function Andor = Andor(CCDSettingTemp, AcquisitionMode, ExposureTime, ReadMode, TriggerMode, PreAmpGain, NumberKinetics, NumberAccumulations,CropHeight,CosmicRayFilterOn)
            ...
            if ret == Shamrock.SHAMROCK_SUCCESS
                ...
                disp('Error occurs during Shamrock initialization! Please check...!')
            end           
                        
            %get physical parameters of system;
            Andor.ReadMode = 0; %Set to FVB mode in order to get the CCD size;
            ...
            if nargin<3
                ExposureTime = 0.2;                    %   ExposureTime in second;
            end            
            Andor.ExposureTime = ExposureTime; 
            
            if nargin<4                        %ReadMode, 0 - Full Vertical Binning, 1 - Multi-Track, 2  - Random-Track, 3 - Single-Track, 4 - Image;
                if Andor.ReadMode ~= 0
                    Andor.ReadMode = 0; %Set to FVB mode in order to get the CCD size;
                end
            else
                if ReadMode ~= 0
                    Andor.ReadMode = ReadMode; %Set to FVB mode in order to get the CCD size;
                end
            end            
            ...
            [ret]=SetShutter(1, 0, 0, 0); %set shutter automatic;
            CheckWarning(ret); 
            
            if nargin < 7 %NumberKinetics 
                NumberKinetics = 1; %by default
            end
            Andor.NumberKinetics = NumberKinetics;
            ...
            if nargin < 9 
                CropHeight = 0;             %No active cropmode
            end
            Andor.CropHeight = CropHeight; 
        ...
        function Andor = set.CurrentGrating(Andor,newGrating)
            Andor.CurrentGrating = SetCurrentGrating(Andor,newGrating);
        end
        function SlitWidth = get.SlitWidth(Andor)
            SlitWidth = GetSlitWidth(Andor);
        end
        function Andor = set.SlitWidth(Andor,newSlitWidth)
            Andor.SlitWidth = SetSlitWidth(Andor,newSlitWidth);
        end
        function CentralWavelength = get.CentralWavelength(Andor)
            CentralWavelength = GetCentralWavelength(Andor);
        end
        function Andor = set.CentralWavelength(Andor,newCentralWavelength)
            Andor.CentralWavelength = SetCentralWavelength(Andor,newCentralWavelength);
        end
        function AxisWavelength = get.AxisWavelength(Andor)
            AxisWavelength = GetAxisWavelength(Andor);
        end
...
%             ShamrockCheckWarning(ret);
        ...
        function Andor = set.CCDSettingTemp(Andor,newCCDSettingTemp)  %Set CCD temperature;
            ...
            if newCCDSettingTemp<mintemp || newCCDSettingTemp>maxtemp 
                disp('Temperature is out of range! Set to -70 bt default!');
                newCCDSettingTemp = -70;
            end
